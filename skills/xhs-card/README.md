# Guizang Social Card · 小红书图文 / 公众号封面对
...
![Guizang Social Card 效果展示](https://github.com/user-attachments/assets/d370abcc-1fc4-4de1-903a-09020a6556ce)
...
```text
帮我安装 xhs-card。请把 https://github.com/op7418/xhs-card 克隆到 ~/.claude/skills/xhs-card,安装完成后检查 SKILL.md、assets/、references/ 是否存在。
```

已经安装过的话,用这段话更新:

```text
帮我更新 xhs-card。请进入 ~/.claude/skills/xhs-card 执行 git pull,然后告诉我当前最新 commit。
```
...
> 帮我安装 `xhs-card` 这个 Claude Code skill。请按下面步骤做:
>
> 1. 确保 `~/.claude/skills/` 目录存在(不存在就创建)
> 2. 执行 `git clone https://github.com/op7418/xhs-card.git ~/.claude/skills/xhs-card`
> 3. 验证:`ls ~/.claude/skills/xhs-card/` 应该看到 `SKILL.md`、`assets/`、`references/` 三项
