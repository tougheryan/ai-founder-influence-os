# Guizang Social Card · Xiaohongshu Carousels / WeChat Cover Pairs

![GitHub stars](https://img.shields.io/github/stars/op7418/xhs-card?style=flat-square)
![License](https://img.shields.io/github/license/op7418/xhs-card?style=flat-square)
...
![Guizang Social Card preview](https://github.com/user-attachments/assets/d370abcc-1fc4-4de1-903a-09020a6556ce)

## 30-second start

```bash
npx skills add https://github.com/op7418/xhs-card --skill xhs-card
```

Or paste this to an AI agent with shell access:

```text
Install xhs-card for me. Clone https://github.com/op7418/xhs-card into ~/.claude/skills/xhs-card, then verify that SKILL.md, assets/, and references/ exist.
```

If you already installed it, update with:

```text
Update xhs-card for me. Go to ~/.claude/skills/xhs-card, run git pull, then tell me the latest commit.
```
...
> Install the `xhs-card` Claude Code skill for me. Steps:
>
> 1. Make sure `~/.claude/skills/` exists (create if not)
> 2. Run `git clone https://github.com/op7418/xhs-card.git ~/.claude/skills/xhs-card`
> 3. Verify: `ls ~/.claude/skills/xhs-card/` should show `SKILL.md`, `assets/`, `references/`
...
```bash
git clone https://github.com/op7418/xhs-card.git ~/.claude/skills/xhs-card
```
